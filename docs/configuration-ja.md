...
| フィールド | 説明 | 例 |
|----------|------|-----|
| `type` | フック種類: `"command"`（スクリプト実行）または `"notification"`（メッセージ表示） | `"notification"` |
| `command` | 実行するシェルコマンド（`type: "command"` 用） | `"node scripts/hooks/session-start.js"` |
| `message` | 表示するメッセージ（`type: "notification"` 用） | `"下書きが作成されました。"` |
...
メール/Slackの直接送信アクション前に発火 — 警告通知を表示し送信をブロック。下書きポリシーを強制。
  ...
  "PreToolUse": [
    {
      ...
      "type": "notification",
      "message": "⚠️ 下書きポリシー: この操作はメッセージを直接送信します。代わりに下書きを作成してください。"
    }
  ]
}
```
...
下書き作成後に発火 — アクションを確認し、手動送信前のレビューを促す。
  ...
  "PostToolUse": [
    {
      ...
      "type": "notification",
      "message": "📝 下書きが作成されました。送信前にご確認ください。"
    }
  ]
}
```
